#!/bin/bash
# Fuzzy-search explicitly-installed packages (yay -Qqe) and remove the picked ones.
# Layout/keybinds mirror Omarchy's omarchy-pkg-remove; colors pull from pywal.
...
printf '\n%s%s  %s%s\n' "$BOLD" "$CYAN" "Removing these packages:" "$RESET"
printf '%s  ─────────────────────────────────────────────%s\n\n' "$CYAN" "$RESET"
