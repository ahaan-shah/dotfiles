...
Scope {
    ...
    // ---- pywal palette -------------------------------------------------------
    // Read from WalColors, the singleton the launcher and the lock screen
    // already read, rather than from a second copy of the same palette in
    // ~/.cache/wal/colors-waybar.css. That file is still generated by pywal and
    // nothing else on this machine reads it; this one stopped when the three
    // shells became one and the duplicate parse became visible.
    //
    // Deliberately still a MAP rather than direct bindings: every colour below
    // and ~200 uses downstream read `palette[...]`, and the point of this change
    // is to remove a parser, not to touch 200 call sites. `text` is absent here
    // exactly as it was absent from the CSS — pywal's waybar template defines
    // foreground/background/cursor and color0-15 and no `text` — so colText
    // still falls through to colFg, unchanged.
    readonly property var palette: ({
        "background": WalColors.background,
        "foreground": WalColors.foreground,
        "color7":     WalColors.color7,
        "color9":     WalColors.color9,
        "color15":    WalColors.color15
    })
    ...
    // From ~/.config/shell that resolves to ~/.config/scripts; from
    ...
    // The FileView that used to sit here is gone. It watched and parsed
    // ~/.cache/wal/colors-waybar.css — a SECOND generated file carrying the
    // same sixteen colours WalColors already reads out of colors.json, with a
    // second watch and a second parser, and it only existed because this file
    // began as a Waybar port whose style.css did `@import colors-waybar.css`.
    //
    // In three processes that was merely wasteful. In one shell it was the same
    // palette parsed twice in the same address space. See `palette` above.
